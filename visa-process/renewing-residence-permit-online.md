...
In addition to the main application form, you must gather and scan all other supporting documents.

**Common documents include:**

1.  **Certificate of Employment (`在職証明書 => zaishoku shoumeisho`):**

    - **Source:** Your company's HR department. Ask them to issue one for your visa renewal. This is _separate_ from the "For Organization" form.

2.  **Tax & Income Certificates (`課税・納税証明書 => kazei/nouzei shoumeisho`):**
...
3.  **Merge PDF:**
    ...
    - Drag and drop them into a logical order (e.g., Application, Passport, Residence Card, Tax Certs).
...
4.  **Upload Your Files:** The system will prompt you to upload:
    - Your Digital ID Photo (from Step 4)
    - Your single, merged, compressed PDF (from Step 5)
5.  **Submit:** Double-check all information and uploaded files, then submit your application. You will get a receipt number (`申請受付番号`).
6.  **Wait for Review:** After submission, your application status will be "under review." This process can take several weeks to a few months.
...
2.  **Payment Form (`手数料納付書 => tesuuryou noufusho`):**
    ...
    ![Payment form fillup guide](../resources/images/visa-process/renew-payment-form.png)
